...
def main():
 ...
 for x in d:
  if x['applicability']['controllers'] not in (['LJ-X8000'],['LJ-S8000']):e.append(x['workflow_id']+': family')
  for s in x['steps']:
 ...
 operations=json.loads((O/'operations/ljs8000_contextual_ui_io_save.yaml').read_text(encoding='utf8'))['records']
 if len({x.get('operation_id') for x in operations})!=len(operations):e.append('duplicate LJ-S contextual operation IDs')
 for x in operations:
  if x.get('family')!='LJ-S8000' or x.get('mode')!='CONTEXT_REQUIRED' or x.get('knowledge_status')!='OFFICIAL_CONTEXTUAL':e.append(x.get('operation_id','?')+': contextual scope')
  p=x.get('legacy_guidance_provenance',{})
  if not p.get('resource_key') or not p.get('source_sha256'):e.append(x.get('operation_id','?')+': provenance')
 ...
 report=f'# Workflow validation\n\nErrors: {len(e)}. Warnings: {len(w)}. Workflows: {len(d)}. LJ-S contextual operations: {len(operations)}.\n';(O/'validation_workflows.md').write_text(report,encoding='utf8');print(json.dumps({'errors':e,'warnings':w,'workflows':len(d),'ljs_contextual_operations':len(operations)}))
